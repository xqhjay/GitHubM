...
import {
  ...
  Cpu, Wrench, ChevronDown, ChevronRight, BrainCircuit, Loader2,
} from 'lucide-react';
...
/**
 * 解析 SSE chunk 数据，返回类型化对象。
 * 后端发送格式：data: {"type":"...","content":"..."}
 * 兼容旧版无 type 字段的 content 格式。
 */
export function parseTypedChunk(data: string): import('./aiTypes').SSEChunk | null {
  if (data === '[DONE]') return null;
  try {
    const parsed = JSON.parse(data) as Record<string, unknown>;
    if (parsed.type) return parsed as unknown as import('./aiTypes').SSEChunk;
    // 兼容旧版：无 type 字段时视为 content
    const text = (parsed as { choices?: Array<{ delta?: { content?: string } }> })
      .choices?.[0]?.delta?.content ?? (parsed.content as string | undefined) ?? '';
    if (text) return { type: 'content', content: text };
    return null;
  } catch {
    return null;
  }
...
/** 可折叠的思考过程块，支持流式输入（done=false 时显示加载动画），可从外部直接使用 */
export function ThinkingBlock({ content, done = true }: { content: string; done?: boolean }) {
    ...
    <div className="my-1 rounded-lg border border-border bg-muted/40 overflow-hidden">
      ...
      >
        {done
          ...
          : <Loader2 className="h-3.5 w-3.5 shrink-0 text-primary/60 animate-spin" />}
        <span className="flex-1 font-medium">{done ? '思考过程' : '思考中…'}</span>
        {done && (open
          ? <ChevronDown className="h-3.5 w-3.5 shrink-0" />
          : <ChevronRight className="h-3.5 w-3.5 shrink-0" />)}
      </button>
      {/* 流式时自动展开，完成后折叠（用户可手动展开） */}
      {(open || !done) && content && (
        <div className="px-3 pb-3 pt-1 border-t border-border text-xs text-muted-foreground leading-relaxed whitespace-pre-wrap break-words max-h-48 overflow-y-auto">
          {content}
          {!done && <span className="inline-block w-1 h-3 bg-muted-foreground ml-0.5 animate-pulse rounded-sm align-middle" />}
        </div>
      )}
    </div>
  );
